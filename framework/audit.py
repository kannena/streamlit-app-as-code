...
class AuditLogger:
    ...
    def _build_entry(self, activity_type: str, session_id: str,
    ...
    def record(self, activity_type: str, session_id: str, **kwargs):
        """Queue an audit entry. Flushes automatically when thresholds are met."""
        if not self.enabled:
            return
        entry = self._build_entry(activity_type, session_id, **kwargs)
        self._enqueue(entry)

    def _enqueue(self, audit_entry: Dict[str, Any]):
        if not self.enabled:
            return
    ...
    def _drain_batch(self):
        ...
        if self.async_logging:
            threading.Thread(target=self._persist_entries, args=(entries,), daemon=True).start()
        else:
...
def log_event(audit_logger, activity_type: str, session_id: str, **kwargs):
    """Wrapper that ensures audit logging never raises exceptions."""
    try:
        if audit_logger and hasattr(audit_logger, 'record'):
            audit_logger.record(activity_type, session_id, **kwargs)
    except Exception:
        pass
