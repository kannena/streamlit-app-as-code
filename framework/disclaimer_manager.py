...
class DisclaimerManager:
    ...
    def should_show_disclaimer(self, username: str, app_name: str) -> bool:
        ...
        if self.persistence_config.get('enabled', False):
            if self._check_persistent_acceptance(username, app_name, version):
                return False
    ...
    def record_acceptance(self, username: str, app_name: str, version: str) -> bool:
        ...
        try:
            ...
            safe_user = self._esc(username)
            safe_app = self._esc(app_name)
            safe_ver = self._esc(version)
            safe_env = self._esc(self.environment)
    ...
    def _check_persistent_acceptance(self, username: str, app_name: str,
                                     version: str) -> bool:
        try:
            table = self._resolved_table()
            ...
            SELECT COUNT(*) AS cnt
            FROM {table}
            WHERE username            = '{self._esc(username)}'
              AND app_name            = '{self._esc(app_name)}'
              AND disclaimer_version  = '{self._esc(version)}'
              AND environment         = '{self._esc(self.environment)}'
