...
def detect_environment(sess, default_env="DEV"):
    try:
...
try:
    from audit import AuditLogger, log_event
    AUDIT_AVAILABLE = True
except ImportError:
    AUDIT_AVAILABLE = False

ENV_CURRENT = detect_environment(session) if session else "DEV"
...
def _resolve_table(table_key: str) -> str:
    ...
    if "{DB}" in schema:
        schema = resolve_placeholders(schema, DB_NAME)
    return f"{schema}.{name}"


catalog_table = _resolve_table("catalog_table")
folder_table = _resolve_table("folder_table")
...
def apply_presets(filter_values: dict):
        ...
        for fname, value in filter_values.items():
            ...
            if itype == "text":
                ...
                if "start_date" in value:
                    st.session_state[f"{fname}_from_date"] = _parse_date(value["start_date"])
                if "end_date" in value:
                    st.session_state[f"{fname}_to_date"] = _parse_date(value["end_date"])
            elif itype == "checkbox":
                st.session_state[f"selected_{fname}"] = value
...
def fetch_folders(user_key=None):
    """Fetch folders the user has access to based on Snowflake roles."""
    try:
        from permissions import get_user_snowflake_roles
...
def children_of(df, parent_id) -> list:
...
def subtree_ids(df, root_id) -> list:
...
@st.cache_data(ttl=600)
def load_user_presets(user_key=None) -> list:
...
def load_preset(name: str, folder_id) -> dict | None:
...
def save_preset(name: str, filter_values: dict, folder_id) -> bool:
...
def delete_preset(name: str, folder_id) -> bool:
...
def check_admin_status() -> bool:
    ...
    try:
        from env_resolver import role_helper_table_name
        current_user = current_user_login()
        sql = f"""
            SELECT COUNT(*) AS cnt
...
def _draw_load_panel():
    ...
    globals_data = load_user_presets(user_key=_user_key)
    if not globals_data:
        st.info("📁 No global filters found.")
        return

    raw_folders = fetch_folders(user_key=_user_key)
    ...
    while True:
        children = children_of(raw_folders, current_parent)
        if not children:
            break
    ...
    valid_ids = set(subtree_ids(raw_folders, active_folder))
...
def _draw_save_panel():
    ...
    _user_key = st.session_state.get("current_user_raw", "")
    raw_folders = fetch_folders(user_key=_user_key)
    folder_paths = build_tree_paths(raw_folders)
    ...
    if st.button("💾 Save Preset", type="primary", disabled=not (preset_name and preset_name.strip())):
        ...
        elif save_preset(preset_name.strip(), current_filters, parent_id):
            st.success(f"✅ Saved preset: '{preset_name.strip()}'")
            st.rerun()


def _draw_manage_panel():
    """Manage tab: list, preview, rename, and delete presets."""
    st.markdown("**Manage Saved Presets**")

    _user_key = st.session_state.get("current_user_raw", "")
    presets = load_user_presets(user_key=_user_key)
    if not presets:
