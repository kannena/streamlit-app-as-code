...
try:
    from audit_logger import StreamlitAuditLogger, safe_audit_log, generate_session_id
...
def load_app_config(path: str = "config.yaml") -> dict:
...
def load_framework_config(path: str = "framework_config.yaml") -> dict:
...
def merge_configs(framework: dict, app: dict) -> dict:
    ...
    for key in all_keys:
        ...
        if isinstance(f_val, dict) and isinstance(a_val, dict):
            merged[key] = merge_configs(f_val, a_val)
...
def load_sql_template(path: str = "queries.sql") -> str:
...
def detect_env(session, default_env: str = "DEV") -> str:
...
def get_user_security_divisions(session, config: dict, db_name: str,
    ...
    query = resolve_db_placeholder(query, db_name)
...
def parse_dependencies(depends_on: list) -> list:
    """
...
def check_dependencies_satisfied(dep_groups: list, filter_values: dict) -> bool:
...
def build_query(sql_template: str, config: dict, filter_values: dict,
    ...
    sql = resolve_db_placeholder(sql, db_name)
...
def render_filters(config: dict, session, db_name: str,
    ...
    _render_filter_group(mandatory, config, session, db_name,
    ...
    if optional:
        ...
        with st.expander(adv_title, expanded=adv_expanded):
            _render_filter_group(optional, config, session, db_name,
                                 filter_values, updated_values, cache_mgr)

    return updated_values


def _render_filter_group(filter_list, config, session, db_name,
                         filter_values, updated_values, cache_mgr):
    ...
    for filter_name, f_config in filter_list:
        ...
        # Check dependencies
        if depends_on:
            dep_groups = parse_dependencies(depends_on)
            if not check_dependencies_satisfied(dep_groups, filter_values):
                updated_values[filter_name] = None
                continue

        # Render based on input type
        if input_type == "checkbox":
            _render_checkbox_filter(
                filter_name, f_config, session, db_name,
                filter_values, updated_values, cache_mgr)

        elif input_type == "date":
            _render_date_filter(filter_name, f_config, updated_values)
...
def _render_checkbox_filter(filter_name, f_config, session, db_name,
                            filter_values, updated_values, cache_mgr):
    ...
    resolved_sql = resolve_db_placeholder(filter_sql, db_name)
...
def _render_date_filter(filter_name, f_config, updated_values):
...
def generate_filter_text_file(config: dict, filter_values: dict,
...
def download_as_csv_zip(df: pd.DataFrame, config: dict,
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        if export_config.get('formats', {}).get('csv', {}).get('include_filter_file', True):
            filter_text = generate_filter_text_file(
...
def main():
    ...
    env = detect_env(session)
    db_name = get_db_for_env(env)
    current_user = get_current_user(session)

    # ── Load Configuration ────────────────────────────────────────────
    framework_config = load_framework_config()
    app_config = load_app_config()
    config = merge_configs(framework_config, app_config)
    ...
    cache_mgr = SessionCacheManager() if CACHE_AVAILABLE else None
    ...
    if AUDIT_AVAILABLE:
        ...
        audit_logger = StreamlitAuditLogger(
            ...
            env, db_name)
        safe_audit_log(audit_logger, 'session_start', session_id)

    # ── Disclaimer ────────────────────────────────────────────────────
    if DISCLAIMER_AVAILABLE and config.get('disclaimer', {}).get('enabled', False):
        dm = DisclaimerManager(session, config, db_name, env, audit_logger)
        if dm.should_show_disclaimer(current_user, app_info.get('title', '')):
            _show_disclaimer_dialog(dm, config, current_user,
                                    app_info.get('title', ''), session_id, audit_logger)
            return  # Block app until accepted

    # ── Security Divisions ────────────────────────────────────────────
    security_divisions = get_user_security_divisions(
        session, config, db_name, current_user, cache_mgr)

    # ── Warehouse Context ─────────────────────────────────────────────
    _ensure_warehouse(session, config, env)
...
def _execute_and_display(session, config, sql_template, filter_values,
                         db_name, current_user, security_divisions,
                         audit_logger, session_id, cache_mgr):
    ...
    # Log query execution
    if audit_logger:
        safe_audit_log(audit_logger, 'query_executed', session_id,
    ...
    if csv_conf.get('enabled', True):
        if st.button(csv_conf.get('label', '📄 Download CSV')):
            with st.spinner("Preparing download..."):
                ...
                if audit_logger:
                    safe_audit_log(audit_logger, 'data_export', session_id,
                                   details={'format': 'csv'},
...
def _show_disclaimer_dialog(dm, config, current_user, app_name,
    ...
    with col1:
        if st.button(ui.get('accept_button', 'I Accept'), type="primary"):
            ...
            dm.record_acceptance(current_user, app_name, version)
            st.session_state[f"disc_ok_{version}_{current_user}"] = True
            if audit_logger:
                safe_audit_log(audit_logger, 'disclaimer_accepted', session_id)
            st.rerun()
    with col2:
...
def _ensure_warehouse(session, config, env):
    """Set the warehouse context from config."""
    try:
        db_config = config.get('database', {})
        wh = db_config.get('warehouse', '')
        if wh:
            wh = resolve_environ_placeholder(wh, env)
        ...
        try:
            ...
            if fallback:
                fallback = resolve_environ_placeholder(fallback, env)
